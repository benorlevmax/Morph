#!/usr/bin/env python3
"""selfplay.py - UCI-driven self-play position generation for a worker.

Self-contained (no dependency on tools/nnue_pipeline/) since a worker is
meant to run on a separate machine that may only have this `distributed/`
directory plus a compiled engine binary -- it does not assume the rest of
this repo is present.

Unlike `chess_train gen`'s bulk bullet-text exporter (used by
tools/nnue_pipeline/generate.py), this drives the engine one move at a time
over the UCI protocol so it can capture the REAL per-position search depth
and node count the distributed dataset schema requires, not just a constant
depth label.
...
used by src/train/dataset.h's Sample struct and tools/nnue_pipeline's schema.
...
score belongs to without tracking the board itself). run_worker.py checks
for it at startup and fails with a clear message if it's missing.
"""
import random
import re
import subprocess
